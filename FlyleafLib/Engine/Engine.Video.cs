...
public class VideoEngine
{
    ...
    private Dictionary<long, GPUAdapter> GetAdapters()
    {
        ...
        for (int i = 0; Factory.EnumAdapters(i, out var adapter).Success; i++)
        {
    ...
    public GPUAdapter GetGPUAdapter(IDXGIAdapter adapter, AdapterDescription desc)
        => new()
        {
            SystemMemory    = (uint)desc.DedicatedSystemMemory,
            VideoMemory     = (uint)desc.DedicatedVideoMemory,
            SharedMemory    = (uint)desc.SharedSystemMemory,
            Vendor          = (GPUVendor)desc.VendorId,
            Description     = desc.Description,
            Id              = (uint)desc.DeviceId,
            Luid            = desc.Luid,
            dxgiAdapter     = adapter
        };
