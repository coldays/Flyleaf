...
public unsafe class SwapChain
{
    ...
    public Result Present()
        => sc.Present((int)ucfg.VSync, PresentFlags.None);

    public Result Present(uint syncInterval, PresentFlags flags)
        => sc.Present((int)syncInterval, flags);
    ...
    internal void SetSize()
    {
        ...
        bbRtv.  Dispose();
        bb.     Dispose();
        sc.     ResizeBuffers(0, (int)vp.ControlWidth, (int)vp.ControlHeight, Format.Unknown, SwapChainFlags.None);
        bb      = sc.GetBuffer<ID3D11Texture2D>(0);
        bbRtv   = Renderer.Device.CreateRenderTargetView(bb);
    ...
    void AddSubClass()
    {
        if (!hasSubClass)
        {
            ...
            if (Environment.CurrentManagedThreadId == Application.Current.Dispatcher.Thread.ManagedThreadId)
                SetWindowSubclass(ControlHwnd, wndProcDelegatePtr, UIntPtr.Zero, UIntPtr.Zero);
            else
                UI(() => SetWindowSubclass(ControlHwnd, wndProcDelegatePtr, UIntPtr.Zero, UIntPtr.Zero));
        }
    }
    void RemoveSubClass()
    {
        if (hasSubClass)
        {
            hasSubClass = false;
            if (Environment.CurrentManagedThreadId == Application.Current.Dispatcher.Thread.ManagedThreadId)
                RemoveWindowSubclass(ControlHwnd, wndProcDelegatePtr, UIntPtr.Zero);
            else
                UI(() => RemoveWindowSubclass(ControlHwnd, wndProcDelegatePtr, UIntPtr.Zero));
        }
    }
