...
public unsafe partial class Renderer : IVP
{
    ...
    VideoProcessors VPSelection()
    {
        ...
        canFL = VideoDecoder.VideoAccelerated ||
            (!scfg.PixelFormatDesc->flags.HasFlag(AV_PIX_FMT_FLAG_PAL) && (!scfg.PixelFormatDesc->flags.HasFlag(AV_PIX_FMT_FLAG_BE) || scfg.PixelComp0Depth <= 8));
        ...
        if (canFL &&
            ...
            (!canD3 || scfg.ColorSpace == ColorSpace.Bt2020 || ucfg.hflip || ucfg.vflip || ucfg.SplitFrameAlphaPosition != SplitFrameAlphaPosition.None || scfg.PixelFormatDesc->flags.HasFlag(AV_PIX_FMT_FLAG_ALPHA)) ||
    ...
    {   // TBR: Cannot run in parallel with Fill Frames!!! (same txt/srv/swsframe etc..)
        ...
        if (mFrame.AVFrame != null) // HW Frame
        {
            ...
            {
                vpivd.Texture2D.ArraySlice = (int)mFrame.AVFrame->data[1];
                mFrame.VPIV = vd.CreateVideoProcessorInputView(ffTexture, ve, vpivd);
            }
            else if (VideoProcessor == VideoProcessors.Flyleaf)
            {
                srvDesc[0].Texture2DArray.FirstArraySlice = srvDesc[1].Texture2DArray.FirstArraySlice = (int)mFrame.AVFrame->data[1];
                mFrame.SRV = [
...
public class VPConfig : NotifyPropertyChanged
{
    ...
    [JsonIgnore]
#endif
    public double           PanXOffset              { get => panXOffset;            set { if (Set(ref panXOffset, Math2.Clamp(value, -10, 10))) vp?.VPRequest(VPRequestType.Viewport); } }
    ...
    [JsonIgnore]
#endif
    public double           PanYOffset              { get => panYOffset;            set { if (Set(ref panYOffset, Math2.Clamp(value, -10, 10))) vp?.VPRequest(VPRequestType.Viewport); } }
