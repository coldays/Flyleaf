...
public unsafe partial class Renderer
{
    ...
    bool D3HWConfig()
    {
        ...
        d3txtDesc.Width = (int)scfg.txtWidth;
        d3txtDesc.Height= (int)scfg.txtHeight;

        return true;
    }
    bool D3SWConfig()
    {
        if (scfg.ColorType == ColorType.RGB && scfg.PixelFormat != AVPixelFormat.AV_PIX_FMT_RGBA)
        ...
        psCase              = PSCase.SWD3;
        d3txtDesc.Width     = (int)(scfg.txtWidth  & ~1u);
        d3txtDesc.Height    = (int)(scfg.txtHeight & ~1u);
...
");
    ...
    VideoFrame D3HWFillPlanes(ref AVFrame* frame)
    {
        ...
        vpivd.Texture2D.ArraySlice = (int)frame->data[1];
    ...
    void D3SetPSY(string uniqueId, string sampleHLSL)
    {
        if (!psShader.TryGetValue(psYId, out var shader))
        {
            shader = ShaderCompiler.CompilePS(device, uniqueId, sampleHLSL.AsSpan());
    ...
    void D3SetPSUV(string uniqueId, string sampleHLSL)
    {
    ...
    void EnableSuperRes()
    {
        ...
        if (GPUAdapter.Vendor == GPUVendor.Nvidia)
            fixed (SuperResNvidia* ptr = &SuperResEnabledNvidia)
                vc.VideoProcessorSetStreamExtension(vp, 0, GUID_SUPERRES_NVIDIA, (int)sizeof(SuperResNvidia), (nint)ptr);
    ...
    void UpdateSuperResIntel(bool enabled)
    {
        ...
        vc.VideoProcessorSetOutputExtension(vp,     GUID_SUPERRES_INTEL, (int)sizeof(SuperResIntel), handle.AddrOfPinnedObject());
        ...
        vc.VideoProcessorSetStreamExtension(vp, 0,  GUID_SUPERRES_INTEL, (int)sizeof(SuperResIntel), handle.AddrOfPinnedObject());
    ...
    void D3Render(VideoFrame frame, bool secondField)
    {
        ...
        */
        ...
        vpsa[0].OutputIndex = vpsa[0].InputFrameOrField = secondField ? 1 : 0;
