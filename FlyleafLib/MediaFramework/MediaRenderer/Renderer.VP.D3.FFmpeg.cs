...
public unsafe partial class Renderer
{
    ...
    {   /* Creates FFDevice which holds our Device/Context and VideoDevice/VideoContext (extra refs)
        ...
        hwCtx->device           = (FFmpeg.AutoGen.ID3D11Device*)        device. NativePointer;
        hwCtx->device_context   = (FFmpeg.AutoGen.ID3D11DeviceContext*) context.NativePointer;
        hwCtx->video_device     = (FFmpeg.AutoGen.ID3D11VideoDevice*)   vd.     NativePointer;
        hwCtx->video_context    = (FFmpeg.AutoGen.ID3D11VideoContext*)  vc.     NativePointer;
    ...
    {   /* (Re-)Allocates FFFrames -if required- and "extracts" the HW Texture Array (extra ref)
        ...
        fixed(AVBufferRef** ptr = &ffFrames)
            ...
            {
                ffFramesInfo.CodecId = AVCodecID.AV_CODEC_ID_NONE;
                Log.Error($"{FFmpegEngine.ErrorCodeToMsg(ret)} ({ret})");
                return false;
            }
    ...
    {   // Checks whether we need to Re-Allocate FFFrames - Called by ConfigHWFrames (Helper)
        ...
        if (     codecId == AVCodecID.AV_CODEC_ID_MPEG2VIDEO)
            surface_alignment = 32;
        else if (codecId == AVCodecID.AV_CODEC_ID_HEVC || codecId == AVCodecID.AV_CODEC_ID_AV1)
        ...
        if (     codecId == AVCodecID.AV_CODEC_ID_H264|| codecId == AVCodecID.AV_CODEC_ID_HEVC)
            num_surfaces += 16;
        else if (codecId == AVCodecID.AV_CODEC_ID_VP9 || codecId == AVCodecID.AV_CODEC_ID_AV1)
            num_surfaces += 8;
        else
            num_surfaces += 2;
