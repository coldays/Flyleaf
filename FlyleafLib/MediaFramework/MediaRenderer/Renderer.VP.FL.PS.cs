...
SampleSplitFrameAlpha("input.Texture.x", "0.5 + (input.Texture.y / 2)"), defines);
    ...
    bool FLSWYUVConfig()
    {
        ...
        if (scfg.PixelPlanes == 1 && ( // No Alpha
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_Y210LE  || // Not tested
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_YUYV422 ||
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_YVYU422 ||
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_UYVY422 ))
        {
            psCase  = PSCase.YUVPacked;
            psId   += ((int)psCase).ToString();

            psData.UVOffset = 1.0f / (scfg.txtWidth >> 1);
            txtDesc[0].Width   = (int)scfg.txtWidth;
            txtDesc[0].Height  = (int)scfg.txtHeight;
...
";
            if (scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_YUYV422 ||
                scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_Y210LE)
            {
                psId += "a";
...
", defines);
            ...
            else if (scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_YVYU422)
            {
                psId += "b";
...
", defines);
            ...
            else if (scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_UYVY422)
            {
                psId += "c";
...
", defines);
        ...
        else if (scfg.PixelPlanes == 2) // No Alpha
        {
            ...
            txtDesc[0].Width    = (int)scfg.txtWidth;
            txtDesc[0].Height   = (int)scfg.txtHeight;
            txtDesc[1].Width    = (int)(scfg.txtWidth  >> scfg.PixelFormatDesc->log2_chroma_w);
            txtDesc[1].Height   = (int)(scfg.txtHeight >> scfg.PixelFormatDesc->log2_chroma_h);
...
SampleSplitFrameAlpha("input.Texture.x", "0.5 + (input.Texture.y / 2)"), defines);
        ...
        else if (scfg.PixelPlanes > 2) // Possible Alpha
        {
            ...
            txtDesc[0].Width    = txtDesc[3].Width = (int)scfg.txtWidth;
            txtDesc[0].Height   = txtDesc[3].Height= (int)scfg.txtHeight;
            txtDesc[1].Width    = txtDesc[2].Width = (int)(scfg.txtWidth  >> scfg.PixelFormatDesc->log2_chroma_w);
            txtDesc[1].Height   = txtDesc[2].Height= (int)(scfg.txtHeight >> scfg.PixelFormatDesc->log2_chroma_h);
...
SampleSplitFrameAlpha("input.Texture.x", "0.5 + (input.Texture.y / 2)");
    ...
    bool FLSWRGBConfig()
    {
        ...
        if (scfg.PixelPlanes == 1 && ( // Possible Alpha
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_0RGB  ||
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_RGB0   ||
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_0BGR  ||
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_BGR0   ||

            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_ARGB   ||
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_RGBA   ||
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_ABGR   ||
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_BGRA   ||

            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_RGBA64LE||
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_BGRA64LE))
        {
...
";
        ...
        // [BGR/RGB]16
            ...
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_RGB444LE||
            scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_BGR444LE))
        {
            ...
            string shader;
            if (scfg.PixelFormat == AVPixelFormat.AV_PIX_FMT_RGB444LE)
            {
                psId += "a";
                shader = @"
...
";
        ...
        else if (scfg.PixelPlanes > 2) // Possible Alpha | TBR: Usually transfer func 'Linear' for > 8-bit which requires pow (*?)
        {
            ...
            for (int i = 0; i < scfg.PixelPlanes; i++)
            {
                txtDesc[i].Width    = (int)scfg.txtWidth;
                txtDesc[i].Height   = (int)scfg.txtHeight;
            }
...
";
    ...
    {
        if ((nint)frame->data[0] != ffTexture.NativePointer)
        {
    ...
    VideoFrame FLSWFillPlanes(ref AVFrame* frame)
    {
        ...
        for (uint i = 0; i < scfg.PixelPlanes; i++)
        {
            subData[0].RowPitch     = (int)frame->linesize[i];
    ...
    void SetPS(string uniqueId, string sampleHLSL, List<string> defines = null)
    {
        ...
        {   // Check global/static cache for Blob
            shader = ShaderCompiler.CompilePS(device, uniqueId, sampleHLSL.AsSpan(), defines);
