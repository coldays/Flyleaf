...
public unsafe class VideoDecoder : DecoderBase
{
    ...
    static CodecSpec FindHWDecoder(AVCodecID id)
    {
        ...
        while ((codec = av_codec_iterate(&opaque)) != null)
        {
            ...
            while((config = avcodec_get_hw_config(codec, i++)) != null)
                if (config->pix_fmt == PIX_FMT_HWACCEL && config->methods.HasFlag(AV_CODEC_HW_CONFIG_METHOD_HW_DEVICE_CTX))
                {
    ...
    protected override bool Setup()
    {
        ...
        checkKeyFrame       = codecCtx->codec_id != AVCodecID.AV_CODEC_ID_AV1 &&
                             (VideoAccelerated ||
                              codecCtx->codec_id != AVCodecID.AV_CODEC_ID_VP8 && codecCtx->codec_id != AVCodecID.AV_CODEC_ID_VP9);
    ...
    protected override void RunInternal()
    {
        ...
        do
        {
            ...
            lock (lockCodecCtx)
            {
                ...
                if (isRecording)
                {
                    if (!recKeyPacketRequired && (packet->flags & AV_PKT_FLAG_KEY) != 0)
                    {
    ...
    public VideoFrame GetFrame(int frameNumber, bool backwards = false)
    {
        ...
        do
        {
            ...
            int ret = av_seek_frame(demuxer.FormatContext, -1, curSeekMcs - curFixSeekDelta, AVSEEK_FLAG_FRAME | AVSEEK_FLAG_BACKWARD);
            const long oneSecondInMicroSeconds = 1000000;
            if (ret < 0)
                ret = av_seek_frame(demuxer.FormatContext, -1, Math.Max((curSeekMcs - oneSecondInMicroSeconds) - curFixSeekDelta, demuxer.StartTime / 10), AVSEEK_FLAG_FRAME);
