...
unsafe partial class Player
{
    ...
    void ScreamerVASD()
    {
        ...
        while (status == Status.Playing)
        {
            ...
            if (requiresBuffering)
            {
                ...
                {
                    framesDisplayedDwmEnd   = (uint)Renderer.SwapChain.GetFrameStatistics().PresentCount - framesDisplayedDwmStart;
                ...
                if (Config.Player.Stats)
                    framesDisplayedDwmStart = (uint)Renderer.SwapChain.GetFrameStatistics().PresentCount;
        ...
        if (Config.Player.Stats)
        {
            ...
            framesDisplayedDwmEnd   = SafeSubstract((uint)Renderer.SwapChain.GetFrameStatistics().PresentCount, framesDisplayedDwmStart);
            framesDisplayedDwm     += framesDisplayedDwmEnd;
            //Video.fpsCurrent        = 0;
            UI(() =>
            {
    ...
    private void CheckLatency()
    {
        ...
        var newSpeed = Math.Max(Math.Round((double)curLatency / Config.Player.MaxLatency, 1,
    ...
    internal (uint, uint) FramesDisplayedDropped()
    {
        ...
        var framesDisplayedDwmCur = framesDisplayedDwm + SafeSubstract((uint)presentCount, framesDisplayedDwmStart);
